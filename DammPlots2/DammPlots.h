...
class DammPlot {
    ...
    private:
        ...
        DammPlot(int dammId, int xSize, const char* title,
                 ...
                 int contraction) : id_(dammId), dim_(1) ;
    ...
    public:
        ...
        void DeclareHistogram1D(int dammId, int xSize, const char* title,
            ...
            if ( !exists(dammId) && !exists(mne) && checkRange(dammId) ) {
                ...
                // Mnemonic is optional and added only if longer then 0
                if (mne.size() > 0)
                    mneList_.insert( pair<string, int>(mne, id) );
                return true;
            } else {
                if (checkRange(dammId) )
                    cerr << "Histogram " << dammId << ", " << mne << " already exists." << endl; 
        ...
        void DeclareHistogram1D(int dammId, int xSize, const char* title, string mne = "") {
            dammId += offset;
            if ( !exists(dammId) && !exists(mne) && checkRange(dammId) ) {
                idList_.insert( pair<int, DammPlot*>(dammId, new DammPlot(dammId + offset_, xSize, title));
                // Mnemonic is optional and added only if longer then 0
        ...
        void DeclareHistogram2D(dammId, int xSize, int ySize,
                    const char* title, int halfWordsPerChan,
