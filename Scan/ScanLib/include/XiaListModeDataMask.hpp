...
public:
    ...
    unsigned int GetFrequency() { return  frequency_; }

    ///Sets the firmware version
    ///@param[in] firmware : The string that we are going to convert to the
...
private:
    ...
    std::string BadMaskErrorMessage(const std::string &func);

    ///Method that converts the given string to the FIRMWARE enum
    ///@param[in] type : A string that we want to convert to the
    /// equivalent FIRMWARE enum type.
    ///@throws invalid_argument when given a string it cannot convert
    ///@return The firmware enum type that we found
