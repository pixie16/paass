...
Globals::Globals(const std::string &file) {
    ...
    try {
        ...
        for (pugi::xml_node_iterator it = global.begin();
            ...
            } else if (std::string(it->name()).compare("OutputPath") == 0) {
                outputPath_ = it->attribute("value").as_string();
            } else
        ...
        for (pugi::xml_node_iterator it = trc.begin(); it != trc.end(); ++it) {
            if (std::string(it->name()).compare("DiscriminationStart") == 0)
                ...
                for (pugi::xml_node_iterator waveit = it->begin();
                    ...
                    waveformRanges_.insert(std::make_pair(
                            waveit->attribute("name").as_string(),
                            std::make_pair(waveit->child("Low").attribute(
                                    "value").as_int(5),
                ...
                for (pugi::xml_node_iterator trapit = it->begin();
                    ...
                    trapFiltPars_.insert(std::make_pair(
        ...
        for (pugi::xml_node_iterator it = fit.begin(); it != fit.end(); ++it) {
            ...
            else if (std::string(it->name()).compare(
                    "SiPmtSigmaBaselineThresh") == 0)
