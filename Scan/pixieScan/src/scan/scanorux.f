...
C
      INTEGER*4    NSEC,ISTAT,KIND,KERR
C
      REAL*4       XV
C
...
C
      IF(KMD.EQ.'    ') GO TO 100       !Ignore blanks
C
...
C
C     ------------------------------------------------------------------
C     Wait for a specified number of seconds
C     ------------------------------------------------------------------
C
  230 CALL MILV(LWD(1,2),NSEC,XV,KIND,KERR)
      IF(KERR.NE.0)   GO TO 1000
      IF(NSEC.LT.1)   NSEC=1
      IF(NSEC.GT.300) NSEC=300
      CALL WAIT(NSEC,2,ISTAT)
      GO TO 100
C
C     ------------------------------------------------------------------
