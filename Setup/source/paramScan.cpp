...
class parInfo {
  public:
	...
	parInfo() : parName(0), startVal(0), stopVal(0), stepSize(0), value(0), numSteps(0), initialVal(0), goodValues(false), debug(false) { }
	
	parInfo(const char *parName_, const double &startVal_, const double &stopVal_, const double &stepSize_) :
	  parName(parName_), startVal(startVal_), stopVal(stopVal_), stepSize(stepSize_), value(0), numSteps(0), initialVal(0), goodValues(false), debug(false) { 
		checkValues();
	}
	
	bool setValues(const char *parName_, const double &startVal_, const double &stopVal_, const double &stepSize_){
		parName = parName_;
		startVal = startVal_;
		stopVal = stopVal_;
		stepSize = stepSize_;
  ...
  private:
	void updateValues(const double &minValue_, const double &minStep_){
		...
		if(debug){
			std::cout << "start=" << tempStart << " ns, stop=" << tempStop << " ns, step=";
		...
		if(debug){
			...
			for(int i = 0; i <= numSteps; i++)
				std::cout << " " << i << ": " << startVal + i*stepSize << " ns\n";
		}
...
int main(int argc, char *argv[]) {
	...
	if (!isTwoDim) std::cout << "Scan time: " << (par1.numSteps * runTime) / 60.0 << "m\n";
	else std::cout << "Scan time: " << (par1.numSteps * par2.numSteps * runTime) / 60.0 << "m\n";
