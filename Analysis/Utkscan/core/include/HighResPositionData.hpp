...
public:
    ...
    bool GetIsValid() const {
      //  if ( T_ft > 0 && T_fb > 0 && T_bt > 0 && T_bb > 0) return (true);
        if ( Q_ft > 0 && Q_fb > 0 && Q_bt > 0 && Q_bb > 0 && !kSaturated) {
          return (true);
        } else {
          return (false);
        }
    ...
    void AddChanEvent(ChanEvent *chIn){
        if(chIn->GetChanID().HasTag("FT") ){
          T_ft = chIn->GetTrace().GetQdc(); 
          F_ft = chIn->GetEnergy(); 
          if(chIn->GetQdc().size()==8)
              Q_ft = chIn->GetQdc().at(0) - chIn->GetQdc().at(1);
        }
        if(chIn->GetChanID().HasTag("FB") ){
          T_fb = chIn->GetTrace().GetQdc(); 
          F_fb = chIn->GetEnergy(); 
          if(chIn->GetQdc().size()==8)
              Q_fb = chIn->GetQdc().at(0) - chIn->GetQdc().at(1);
        }
        if(chIn->GetChanID().HasTag("BB") ){
          T_bb = chIn->GetTrace().GetQdc(); 
          F_bb = chIn->GetEnergy(); 
          if(chIn->GetQdc().size()==8)
              Q_bb = chIn->GetQdc().at(0) - chIn->GetQdc().at(1);
        }
        if(chIn->GetChanID().HasTag("BT") ){
          T_bt = chIn->GetTrace().GetQdc(); 
          F_bt = chIn->GetEnergy(); 
          if(chIn->GetQdc().size()==8)
              Q_bt = chIn->GetQdc().at(0) - chIn->GetQdc().at(1);
        }
