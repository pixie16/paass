...
//        return (std::numeric_limits<double>::quiet_NaN());
	...
	return (sqrt(GetRPixieQdc() * GetLPixieQdc()));
    }
    ...
    double GetLPixieQdc() const{
        if (left_.GetQdc().size() != 0)
            return left_.GetQdc().at(0) - left_.GetQdc().at(1);
        else
            return -9999;        
    }

    double GetRPixieQdc() const{
        if (right_.GetQdc().size() != 0)
            return right_.GetQdc().at(0) - right_.GetQdc().at(1);
        else
            return -9999;        
    }
