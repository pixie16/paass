...
//TODO We need to make this sensative to running on something other than a 250MHz, also in the logic processor plotting its self
    ...
    if(TreeCorrelator::get()->checkPlace(BEAMNAME)){
        ...
        if (TreeCorrelator::get()->place(BEAMNAME)->status()){
            LogStruc.lastBeamOnTime = TreeCorrelator::get()->place(BEAMNAME)->last().time;
            LogStruc.lastBeamOffTime = TreeCorrelator::get()->place(BEAMNAME)->secondlast().time ;
        } else {
            LogStruc.lastBeamOnTime = TreeCorrelator::get()->place(BEAMNAME)->secondlast().time;
            LogStruc.lastBeamOffTime = TreeCorrelator::get()->place(BEAMNAME)->last().time;
        }
    }
    ...
    if(TreeCorrelator::get()->checkPlace(TAPEMOVENAME)){
        ...
        if (TreeCorrelator::get()->place(TAPEMOVENAME)->status()){
            LogStruc.lastTapeMoveStartTime = TreeCorrelator::get()->place(TAPEMOVENAME)->last().time;
        } else {
            LogStruc.lastTapeMoveStartTime = TreeCorrelator::get()->place(TAPEMOVENAME)->secondlast().time;
        }
    }
    ...
    if(TreeCorrelator::get()->checkPlace(CYCLENAME)){
        ...
        if (TreeCorrelator::get()->place(CYCLENAME)->status()){
            double currentTime_ = TreeCorrelator::get()->place(CYCLENAME)->last().time;
            ...
            LogStruc.lastTapeCycleStartTime = TreeCorrelator::get()->place(CYCLENAME)->secondlast().time;
            LogStruc.cycleNum = tapeCycleNum_;
        }
    }
    
    if(TreeCorrelator::get()->checkPlace(PROTONNAME) && TreeCorrelator::get()->place(PROTONNAME)->status()){
        LogStruc.lastProtonPulseTime = TreeCorrelator::get()->place(PROTONNAME)->last().time;
    }
    
    if (TreeCorrelator::get()->checkPlace(SUPERCYCLENAME) ){
        if (TreeCorrelator::get()->place(SUPERCYCLENAME)->status()){
            LogStruc.lastSuperCycleTime = TreeCorrelator::get()->place(SUPERCYCLENAME)->last().time;
        } else {
            LogStruc.lastSuperCycleTime = TreeCorrelator::get()->place(SUPERCYCLENAME)->secondlast().time;
        }
    }
    //fill the vector
