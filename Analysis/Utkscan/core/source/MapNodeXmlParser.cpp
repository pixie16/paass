...
void MapNodeXmlParser::ParseNode(DetectorLibrary *lib) {
    ...
    bool isVerboseTree =
            XmlInterface::get()->GetDocument()->child("Configuration").child("Tree").attribute("verbose").as_bool(false);
    ...
    for (pugi::xml_node module = map.child("Module"); module; module = module.next_sibling("Module")) {
        ...
        for (pugi::xml_node channel = module.child("Channel"); channel; channel = channel.next_sibling("Channel")) {
            ...
            if (channel.attribute("location").as_int(-1) == -1)
                id.SetLocation(lib->GetNextLocation(id.GetType(), id.GetSubtype()));
            else
                id.SetLocation(channel.attribute("location").as_uint());

            if (isVerbose)
                sstream_ << "Channel " << channelNumber << " = " << id.GetType() << ":" << id.GetSubtype()
