...
drr_entry *OutputHisFile::find_drr_in_list(unsigned int hisID_) {
    ...
    for (std::vector<drr_entry *>::iterator iter = drr_entries.begin();
         iter != drr_entries.end(); iter++) {
    ...
    for (std::vector<unsigned int>::iterator iter = failed_fills.begin();
...
size_t OutputHisFile::push_back(drr_entry *entry_) {
    ...
    if (find_drr_in_list(entry_->hisID)) {
        if (debug_mode) {
            std::cout << "debug: His id = " << entry_->hisID
    ...
    if (debug_mode) {
        std::cout << "debug: Extending .his file by " << entry_->total_size
                  << " bytes for his ID = " << entry_->hisID << " i.e. '"
                  << rstrip(entry_->title) << "'\n";
    }
...
bool OutputHisFile::Finalize(bool make_list_file_/*=false*/,
    ...
    if (debug_mode) {
        std::cout << "debug: NHIS = " << drr_entries.size() << std::endl;
    }
    ...
    if (list_file.good()) {
        int temp_count = 0;
        list_file << std::setw(7) << drr_entries.size() << " HISTOGRAMS,"
...
bool OutputHisFile::Fill(unsigned int hisID_, unsigned int x_, unsigned int y_,
    ...
    if (temp_drr) {
        ...
        if (!temp_drr->find_bin((unsigned int) (x_ / temp_drr->comp[0]),
                                (unsigned int) (y_ / temp_drr->comp[1]),
                                bin)) { return false; }
...
void OutputHisFile::Close() {
    ...
    if (log_file.good()) {
        ...
        for (std::vector<drr_entry *>::iterator iter = drr_entries.begin();
             iter != drr_entries.end(); iter++) {
            log_file << std::setw(5) << (*iter)->hisID << std::setw(10)
                     << (*iter)->total_counts << std::setw(10)
                     << (*iter)->good_counts << std::endl;
        }
