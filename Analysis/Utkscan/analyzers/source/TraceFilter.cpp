...
unsigned int TraceFilter::CalcFilters(const Trace *sig) {
    try {
        ...
        for (vector<unsigned int>::iterator it = trigs_.begin();
             it != trigs_.end(); it++) {
        ...
        switch (errcode) {
            ...
            case (LATE_TRIG) :
                cerr
                        << "The trigger came too late in the trace! I cannnot perform "
                        << "the sums over the necessary ranges, giving zero energy!!"
                        << endl;
                break;
            case (EARLY_TRIG) :
                cerr
                        << "The trigger was too early. Could not calculate baseline "
                        << "or Energy Filter Limits." << endl;
                break;
            case (BAD_FILTER_COEFF):
                cerr << "One of the energy filter coefficients was nan."
                     << endl;
                break;
...
void TraceFilter::CalcTriggerFilter(void) {
    ...
    if (isVerbose_) {
        ...
        if (trigs_.size() > 1)
            cout << "There was(were) " << trigs_.size()
                 << " trigger(s) in the trace." << endl;
        cout << endl;
    }
}
...
void TraceFilter::ConvertToClockticks(void) {
    if (fmod(t_.GetRisetime(), nsPerSample_) != 0) {
        if (isVerbose_)
            cout
                    << "TriggerRisetime is NOT an integer number of samples. Fixing."
                    << endl;
    ...
    if (fmod(e_.GetRisetime(), nsPerSample_) != 0) {
        if (isVerbose_)
            cout
                    << "EnergyRisetime is NOT an integer number of samples.  Fixing."
                    << endl;
    ...
    if (isVerbose_) {
        ...
        cout << "Trigger Filter:" << endl
             ...
             << "Energy Filter: " << endl
             << "  Energy rise (ns): " << e_.GetRisetime() * nsPerSample_
             << endl
             << "  Energy flat (ns): " << e_.GetFlattop() * nsPerSample_ << endl
             << "  Tau(ns) :         " << e_.GetT() * nsPerSample_ << endl
             << endl;
    }
    isConverted_ = true;
}
