...
bool DoubleBetaProcessor::PreProcess(RawEvent &event) {
    ...
    for (auto it = lrtbars_.begin(); it != lrtbars_.end(); it++) {
        ...
        if (DetectorDriver::get()->GetSysRootOutput()){
            ...
            //record time in ns, LRTBarMap uses the Walkcorrected Time which uses GetTimeInNs()
            DBstruc.timeAvg = (*it).second.first ; 
    ...
    for (BarMap::const_iterator it = bars_.begin(); it != bars_.end(); it++) {
        ...
        if (DetectorDriver::get()->GetSysRootOutput()){
            ...
            DBstruc.timeL = (*it).second.GetLeftSide().GetTimeSansCfdInNs(); //store ns;
            DBstruc.timeR = (*it).second.GetRightSide().GetTimeSansCfdInNs(); //store ns;
