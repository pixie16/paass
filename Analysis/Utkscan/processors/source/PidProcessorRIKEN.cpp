...
bool PidProcessor::PreProcess(RawEvent &event) {
   ...
   static const vector<ChanEvent *> &up_scintL_vec = event.GetSummary("pid:up_scint_L_logic", true)->GetList();
   static const vector<ChanEvent *> &up_scintR_vec = event.GetSummary("pid:up_scint_R_logic", true)->GetList();
   static const vector<ChanEvent *> &down_scintL_vec = event.GetSummary("pid:down_scint_L_logic", true)->GetList();
   static const vector<ChanEvent *> &down_scintR_vec = event.GetSummary("pid:down_scint_R_logic", true)->GetList();
   //ionization chamber for dE
   static const vector<ChanEvent *> &ic0_vec = event.GetSummary("pid:ic0", true)->GetList();
   static const vector<ChanEvent *> &ic1_vec = event.GetSummary("pid:ic1", true)->GetList();
   static const vector<ChanEvent *> &ic2_vec = event.GetSummary("pid:ic2", true)->GetList();
   static const vector<ChanEvent *> &ic3_vec = event.GetSummary("pid:ic3", true)->GetList();
   static const vector<ChanEvent *> &ic4_vec = event.GetSummary("pid:ic4", true)->GetList();
   static const vector<ChanEvent *> &ic5_vec = event.GetSummary("pid:ic5", true)->GetList();
   ...
   if(!up_scintR_vec.empty()){
      ...
      up_scintR_time = (*up_scintR)->GetHighResTimeInNs();
      if(root_output){
   ...
   if(!down_scintL_vec.empty()){
      ...
      down_scintL_time = (*down_scintL)->GetHighResTimeInNs();
      if(root_output){
   ...
   if(!down_scintR_vec.empty()){
      ...
      down_scintR_time = (*down_scintR)->GetHighResTimeInNs();
      if(root_output){
   ...
   if(!ic0_vec.empty()){
      ...
      ic0_time = (*ic0)->GetHighResTimeInNs();
      if(ic0_energy>0){
   ...
   if(!ic1_vec.empty()){
      ...
      ic1_time = (*ic1)->GetHighResTimeInNs();
      if(ic1_energy>0){
   ...
   if(!ic2_vec.empty()){
      ...
      ic2_time = (*ic2)->GetHighResTimeInNs();
      if(ic2_energy>0){
   ...
   if(!ic3_vec.empty()){
      ...
      ic3_time = (*ic3)->GetHighResTimeInNs();
      if(ic3_energy>0){
   ...
   if(!ic4_vec.empty()){
      ...
      ic4_time = (*ic4)->GetHighResTimeInNs();
      if(ic4_energy>0){
   ...
   if(!ic5_vec.empty()){
      ...
      ic5_time = (*ic5)->GetHighResTimeInNs();
      if(ic5_energy>0){
   ...
   if(up_scintL_time>0 && up_scintR_time>0 && down_scintL_time>0 && down_scintR_time>0){
      ...
      tof0_flip = -0.5*(down_scintL_time+down_scintR_time)-0.5*(up_scintL_time+up_scintR_time);
      if(root_output){
         pid_struct.tof0 = tof0;
      }
   }
   ...
   //plot(DD_TOF0_PIN0, tof0, pin0_energy);//temporarily commented out for RIKEN PID
   //plot(DD_TOF0_PIN0_FLIP, tof0_flip, pin0_energy);
   plot(DD_TOF0_PIN0, tof0, ic_avg);//temporarily commented out for RIKEN PID
   plot(DD_TOF0_PIN0, tof0_flip, ic_avg);//temporarily commented out for RIKEN PID
   ...
   if (FIT_Implant) {
      ...
      plot(DD_TOF0_PIN0_GATED_FIT_FLIP, tof0_flip, ic_avg);
   ...
   if (YSO_Implant) {
      ...
      plot(DD_TOF0_PIN0_GATED_YSO_FLIP, tof0_flip, ic_avg);
   ...
   if (RIT_Implant) {
      ...
      plot(DD_TOF0_PIN0_GATED_RIT_FLIP, tof0_flip, ic_avg);
