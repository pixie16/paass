...
void XiaListModeDataDecoder::DecodeWordTwo(const unsigned int &word, XiaData &data, const XiaListModeDataMask &mask) {
    ...
    if (mask.GetFrequency() != 500){
        ...
        data.SetCfdTriggerSourceBit(((word & mask.GetCfdTriggerSourceMask().first) >> mask.GetCfdTriggerSourceMask().second));
    } else {
        int CfdSourceVal = (( word & mask.GetCfdTriggerSourceMask().first) >> mask.GetCfdTriggerSourceMask().second);
        if (CfdSourceVal == 7){
            data.SetCfdForcedTriggerBit(true);
        } else {
            data.SetCfdTriggerSourceBit(CfdSourceVal);
            data.SetCfdForcedTriggerBit(false);
        }
    }
}
