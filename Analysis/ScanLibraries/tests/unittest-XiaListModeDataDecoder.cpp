...
CHECK_EQUAL(empty_buffer[1],
        DecodeBuffer(&empty_module_buffer[0], mask)
.
...
CHECK_EQUAL((
unsigned int)0,
DecodeBuffer(&header_w_bad_headerlen[0], mask
).
...
CHECK_EQUAL(slotId, result_data
.

GetSlotNumber()

);
CHECK_EQUAL(channelNumber, result_data
.

GetChannelNumber()

);
CHECK_EQUAL(energy, result_data
.

GetEnergy()

);
CHECK_EQUAL(ts_high, result_data
.

GetEventTimeHigh()

);
CHECK_EQUAL(ts_low, result_data
.

GetEventTimeLow()

);
CHECK_EQUAL(ts, result_data
.
...
CHECK_ARRAY_EQUAL(unittest_trace_variables::trace, result
.

GetTrace(),
        unittest_trace_variables::trace
...
CHECK_EQUAL(cfd_fractional_time, result
.

GetCfdFractionalTime()

);
CHECK_CLOSE(ts_w_cfd, result
.
