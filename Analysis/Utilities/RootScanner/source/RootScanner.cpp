...
//		padZoomInfo->rangeUserMax[2] = pad->GetUzmax();
    ...
    for (int i = 0; i < numAxes_; i++) {
        userZoom[i] = (
                padZoomInfo->rangeUserMin[i] > padZoomInfo->limitMin[i] ||
    ...
    for (TObject *obj = list->First(); obj; obj = list->After(obj)) {
        ...
        if (TH1 *hist = dynamic_cast<TH1 *>(obj)) {
            ...
            if (dynamic_cast<TH2 *>(hist)) {
                ...
                if (hist->GetBinContent(hist->GetMaximumBin()) * 1.1 >
                    padZoomInfo->limitMax[1]) {
                    padZoomInfo->limitMax[1] =
                            1.1 * hist->GetBinContent(hist->GetMaximumBin());
                }
            }
        }
    ...
    for (TObject *obj = list->First(); obj; obj = list->After(obj)) {
        ...
        if (TH1 *hist = dynamic_cast<TH1 *>(obj)) {
            ...
            if (limitChange) {
                //Set the axes limits
                xAxis->SetLimits(padZoomInfo->limitMin[0],
                                 padZoomInfo->limitMax[0]);
                yAxis->SetLimits(padZoomInfo->limitMin[1],
                                 padZoomInfo->limitMax[1]);

                //Check if hist is 2D
                if (dynamic_cast<TH2 *>(hist)) {
                    TAxis *zAxis = hist->GetZaxis();
                    zAxis->SetLimits(padZoomInfo->limitMin[2],
                                     padZoomInfo->limitMax[2]);
                }
        ...
        xAxis->SetRangeUser(padZoomInfo->rangeUserMin[0],
                            padZoomInfo->rangeUserMax[0]);
        yAxis->SetRangeUser(padZoomInfo->rangeUserMin[1],
                            padZoomInfo->rangeUserMax[1]);
    }
