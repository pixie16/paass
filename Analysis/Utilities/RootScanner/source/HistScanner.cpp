...
void HistScanner::Notify(const std::string &code_/*=""*/) {
    if (code_ ==
        "START_SCAN") { lastRefresh_ = std::chrono::system_clock::now(); }
 ...
 */
bool HistScanner::ExtraCommands(const std::string &cmd,
...
void HistScanner::PlotCommand(const std::vector<std::string> &args) {
    ...
    while (stopPos < expr.length()) {
        ...
        if (stopPos != expr.length() &&
            expr.find_first_of(":", stopPos + 1) != std::string::npos) {
            std::cout
                    << "ERROR: Only 1D and 2D plots are currently supported.\n";
        ...
        if (formula.GetNdim() == 0) {
            ...
            std::cout << "(" << split << ").\n";
            std::cout
                    << "Check if mod / chan assignment makes sense with expr.\n";
    ...
    newHists_.push_back(
            std::make_pair(std::make_tuple(expr, weight.str()), pad));
}
...
void HistScanner::RefreshCommand(const std::vector<std::string> &args) {
    ...
    if (args.empty()) {
        ...
        if (refreshDelaySec_)
            std::cout << "Refreshing. Time between auto refreshes is "
                      << refreshDelaySec_ << " s.\n";
    } else if (args.size() == 1) {
        float refreshDelaySec = 0;
        try { refreshDelaySec = std::stof(args[0]); }
        catch (const std::invalid_argument &ia) {
            std::cout << "ERROR: Invalid refresh delay value: '"
                      << refreshDelaySec << "'\n";
            return;
        }
        if (refreshDelaySec < 0) {
            ...
            std::cout << "Auto refresh delay set to " << refreshDelaySec_
                      << " s.\n";
        }
    }
}
...
void HistScanner::Plot(HistKey_ key, TVirtualPad *pad /*= gPad*/) {
    static const std::vector<Color_t> colors = {kBlue + 2, kRed, kGreen + 1,
                                                kMagenta + 2};
    ...
    if (hist) {
        ...
        tree_->Draw(drawCmd.str().c_str(), drawWeight.str().c_str(),
                    drawOpt.str().c_str(), std::numeric_limits<Long64_t>::max(),
                    treeEntries_[hist]);
                    ...
                    for (auto itr = histMap->begin();
                        ...
                        if (hist) {
                            hist->SetLineColor(colors.at(
                                    std::distance(histMap->begin(), itr)));
