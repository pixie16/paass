...
double LoureiroFitFunc(double *x, double *p) {
	//Compute the time difference between x and the phase corrected for clock ticks.
	float diff = (x[0] - p[2])/ADC_TIME_STEP;
	//If the difference is less than zero we return the baseline.
	if (diff < 0 ) return p[0];
	//Return the computed function.
	return p[0] + p[1] * exp(-diff * p[3]) * (1 - exp(-pow(diff * p[4],p[5])));
}
...
void scopeUnpacker::ProcessRawEvent(ScanInterface *addr_/*=NULL*/){
	...
	while(!rawEvent.empty()){
		...
		double maximum =
                ...
                //DPL
		//std::cout<<"scopeUnpacker::ProcessRawEvent() "<<current_event->GetModuleNumber()<<std::endl;
...
void scopeScanner::Plot(){
	...
	//DPL
	//std::cout<<"scopeScanner::Plot()->traceSize "<< traceSize <<std::endl;
...
bool scopeScanner::ExtraCommands(const string &cmd_, vector<string> &args_){
	if(cmd_ == "set"){ // Toggle debug mode
		...
		if (args_.size() == 1) {
			...
			cout << msgHeader << " -SYNTAX- thresh <lowerThresh> [upperThresh]\n";
		}
	}
	else if (cmd_ == "fit") {
		...
		if(performCfd_)
			cout << msgHeader << "Enabling cfd analysis with F=" << cfdF_ << ", D=" << cfdD_ << ", L=" << cfdL_ << endl;
	}
	else if (cmd_ == "avg") {
		if (args_.size() == 1) {
