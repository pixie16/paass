...
private:
    ...
    void Filter(float *trace_, const size_t &length_, float *filtered1,
                const unsigned int &risetime_, const unsigned int &flattop_);
...
public:
    ...
    void PrintStatus(std::string prefix_ = "") {
        std::cout << prefix_ << "Found " << num_traces
