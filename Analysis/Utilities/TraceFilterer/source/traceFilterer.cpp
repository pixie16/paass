...
void Filterer::Filter(float *trace_, const size_t &length_, float *filtered1,
...
void Filterer::ResetGraph(unsigned int size) {
    if (size != x_vals.size()) {
        std::cout << message_head << "Changing trace length from "
                  << x_vals.size() * ADC_TIME_STEP << " to "
                  << size * ADC_TIME_STEP << " ns.\n";
        ...
        while ((unsigned int) graph->GetN() > size)
            graph->RemovePoint(graph->GetN());
    }
...
void Filterer::Plot(ChannelEvent *event_) {
    ...
    Filter(event_->yvals, event_->size, slow_filter_y, energy_rise,
           energy_flat);
    ...
    if (graph->GetYaxis()->GetXmax() > axisMax)
        axisMax = graph->GetYaxis()->GetXmax();
    if (graph->GetYaxis()->GetXmin() < axisMin)
        axisMin = graph->GetYaxis()->GetXmin();

    if (f_fast->GetYaxis()->GetXmax() > axisMax)
        axisMax = f_fast->GetYaxis()->GetXmax();
    if (f_fast->GetYaxis()->GetXmin() < axisMin)
        axisMin = f_fast->GetYaxis()->GetXmin();

    if (f_slow->GetYaxis()->GetXmax() > axisMax)
        axisMax = f_slow->GetYaxis()->GetXmax();
    if (f_slow->GetYaxis()->GetXmin() < axisMin)
        axisMin = f_slow->GetYaxis()->GetXmin();
...
void Filterer::ProcessRawEvent() {
    ...
    while (!rawEvent.empty()) {
        ...
        if (current_event->modNum == mod_ && current_event->chanNum == chan_) {
            ...
            //Check threhsold.
            if (threshHigh_ > threshLow_ &&
                channel_event->maximum > threshHigh_) {
                delete channel_event;
                continue;
            }
...
bool Filterer::Initialize(std::string prefix_) {
    ...
    std::cout << prefix_ << "Displaying traces for mod = " << mod_
              << ", chan = " << chan_ << ".\n";
...
bool Filterer::CommandControl(std::string cmd_,
    ...
    if (cmd_ == "set") { // Toggle debug mode
        ...
        if (args_.size() == 1) {
            ...
            std::cout << message_head << "Set TRIGGER_RISETIME to " << trig_rise
                      << " ADC ticks (" << trig_rise * ADC_CLOCK_uSEC
                      << " μs).\n";
        } else {
            std::cout << message_head << "TRIGGER_RISETIME = "
                      << trig_rise * ADC_CLOCK_uSEC << " μs.\n";
        }
    } else if (cmd_ == "tflat") { // Set the TRIGGER_FLATTOP.
        if (args_.size() == 1) {
            float temp = atof(args_.at(0).c_str());
            trig_flat = (int) (temp / ADC_CLOCK_uSEC);
            std::cout << message_head << "Set TRIGGER_FLATTOP to " << trig_flat
                      << " ADC ticks (" << trig_flat * ADC_CLOCK_uSEC
                      << " μs).\n";
        } else {
            std::cout << message_head << "TRIGGER_FLATTOP = "
                      << trig_flat * ADC_CLOCK_uSEC << " μs.\n";
        }
    } else if (cmd_ == "erise") { // Set the ENERGY_RISETIME.
        if (args_.size() == 1) {
            float temp = atof(args_.at(0).c_str());
            energy_rise = (int) (temp / ADC_CLOCK_uSEC);
            std::cout << message_head << "Set ENERGY_RISETIME to "
                      << energy_rise << " ADC ticks ("
                      << energy_rise * ADC_CLOCK_uSEC << " μs).\n";
        } else {
            std::cout << message_head << "ENERGY_RISETIME = "
                      << energy_rise * ADC_CLOCK_uSEC << " μs.\n";
        }
        ...
        if (args_.size() == 1) {
            ...
            std::cout << message_head << "Set ENERGY_FLATTOP to " << energy_flat
                      << " ADC ticks (" << energy_flat * ADC_CLOCK_uSEC
                      << " μs).\n";
        } else {
            std::cout << message_head << "ENERGY_FLATTOP = "
                      << energy_flat * ADC_CLOCK_uSEC << " μs.\n";
        }
    } else { return false; }
